    ...
    func setSelectedFields() {
        if let order = presenter.filter.order {
            if let index = presenter.filterValues.order.firstIndex(where: { $0 == order }) {
                self.fieldsCollectionView.selectItem(at: IndexPath(item: index, section: 0), animated: false, scrollPosition: [])
            }
        }
        if !presenter.filter.contentRating.isEmpty {
            presenter.filter.contentRating.forEach { item in
                if let index = presenter.filterValues.contentRating.firstIndex(where: { $0 == item }) {
                    self.fieldsCollectionView.selectItem(at: IndexPath(item: index, section: 1), animated: false, scrollPosition: [])
                }
            }
        }
        if !presenter.filter.status.isEmpty {
            presenter.filter.status.forEach { item in
                if let index = presenter.filterValues.status.firstIndex(where: { $0 == item }) {
                    self.fieldsCollectionView.selectItem(at: IndexPath(item: index, section: 2), animated: false, scrollPosition: [])
                }
            }
        }
    }
